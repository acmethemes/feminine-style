 ...
 */
function feminine_style_sanitize_social_data( $input ){
	...
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				...
				elseif ( $key == 'checkbox' ){
					$input_decoded[$boxes][$key] = feminine_style_sanitize_checkbox( $value );
                }
                else{
 ...
 * @return string json encoded
 */
function feminine_style_sanitize_slider_data( $input ){
	$input_decoded = json_decode( $input, true );
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				if( $key == 'selectpage'){
					$input_decoded[$boxes][$key] = feminine_style_sanitize_page( $value );
				}
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Feminine_Style_Customize_Icons_Control' )):
	/**
	 ...
	 *
	 */
	class Feminine_Style_Customize_Icons_Control extends WP_Customize_Control {
            ...
            <label>
                ...
                <span class="at-customize-icons">
                    <span class="icon-preview">
                        <?php if( !empty( $value ) ) { echo '<i class="'. esc_attr( $value ) .'"></i>'; } ?>
                    </span>
                    <span class="icon-toggle">
                        <?php echo ( empty( $value )? esc_html__('Add Icon','feminine-style'): esc_html__('Edit Icon','feminine-style') ); ?>
                        <span class="dashicons dashicons-arrow-down"></span>
                    </span>
                    <span class="icons-list-wrapper hidden">
                        <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','feminine-style')?>">
	                    <?php
	                    $fa_icon_list_array = feminine_style_icons_array();
	                    foreach ( $fa_icon_list_array as $icon => $single_icon ) {
		                    if( $value == $single_icon ) {
			                    echo '<span class="single-icon selected"><i class="'. esc_attr( $icon ) .'"></i></span>';
		                    } else {
			                    echo '<span class="single-icon"><i class="'. esc_attr( $icon ) .'"></i></span>';
		                    }
	                    }
	                    ?>
                    </span>
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Feminine_Style_Repeater_Control' )):
	/**
	 ...
	 *
	 */
	class Feminine_Style_Repeater_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue(){
			...
			wp_enqueue_style( 'feminine-style-customizer-repeater-style', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.css', array(), '3.3.6' );
			wp_enqueue_script( 'feminine-style-customizer-repeater-script', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.js', array('jquery', 'jquery-ui-draggable' ), '1.0.0', true  );
		}
                    ...
                    <div class="repeater-fields hidden">
								...
								switch ( $field['type'] ) {
									...
									case 'icons':
                                        ...
                                        <span class="at-customize-icons">
                                            ...
                                            <span class="icon-toggle">
                                                <?php esc_html_e('Add Icon','feminine-style'); ?>
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </span>
                                            <span class="icons-list-wrapper hidden">
                                                <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','feminine-style')?>">
									...
									case 'image':
                                        ...
                                        <span class="img-preview-wrap" <?php echo  $feminine_style_display_none ; ?>>
                                            <img class="widefat" src="<?php echo esc_url( $new_value ); ?>" alt="<?php esc_attr_e( 'Image preview', 'feminine-style' ); ?>"  />
                                        ...
                                        <input type="button" value="<?php esc_attr_e( 'Upload Image', 'feminine-style' ); ?>" class="button media-image-upload" data-title="<?php esc_attr_e( 'Select Image','feminine-style'); ?>" data-button="<?php esc_attr_e( 'Select Image','feminine-style'); ?>"/>
                        ...
                        <div class="clearfix repeater-footer">
                            <a class="repeater-field-remove" href="#remove">
								<?php esc_html_e( 'Delete', 'feminine-style' ); ?>
                            </a>
	                        <?php esc_html_e( '|', 'feminine-style' ); ?>
                            <a class="repeater-field-close" href="#close">
								<?php esc_html_e( 'Close', 'feminine-style' ); ?>
                            </a>
                        </div>
                    </div>
                </li>
                                ...
                                <div class="single-field type-<?php echo esc_attr( $field['type'] ) . ' ' . esc_attr( $class ); ?>">
									...
									switch ( $field['type'] ) {
										...
										case 'icons':
                                            ...
                                            <span class="at-customize-icons">
                                                <span class="icon-preview">
                                                    <?php if( !empty( $new_value ) ) { echo '<i class="'. esc_attr( $new_value ) .'"></i>'; } ?>
                                                </span>
                                                <span class="icon-toggle">
                                                    <?php echo ( empty( $new_value )? esc_html__('Add Icon','feminine-style'): esc_html__('Edit Icon','feminine-style') ); ?>
                                                    <span class="dashicons dashicons-arrow-down"></span>
                                                </span>
                                                <span class="icons-list-wrapper hidden">
                                                    <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','feminine-style');?>">

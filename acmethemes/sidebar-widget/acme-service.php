 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Feminine_Style_Service' ) ) {

	class Feminine_Style_Service extends WP_Widget {
		...
		function __construct() {
			...
			/*Base ID of your widget*/
				...
				esc_html__( 'AT Service Section', 'feminine-style' ),
				/*Widget description*/
				array(
					'description' => esc_html__( 'Show Section with beautiful Icons.', 'feminine-style' )
				)
			);
		}
		...
		public function form( $instance ) {
            ...
            <br/>
            <small><?php esc_html_e( 'Add Page, Reorder and Remove. Please do not forget to add Icon and Excerpt on selected pages.', 'feminine-style' ); ?></small>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'content_from' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'content_from' ) ); ?>">
					<?php
					$feminine_style_service_content_from = feminine_style_content_from();
					foreach ( $feminine_style_service_content_from as $key => $value ) {
						?>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'column_number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'column_number' ) ); ?>">
					<?php
					$feminine_style_service_column_numbers = feminine_style_widget_column_number();
					foreach ( $feminine_style_service_column_numbers as $key => $value ) {
						?>
			...
			if( isset($new_instance['at_all_page_items'] )){
				...
				if  ( is_array($at_all_page_items) && count($at_all_page_items) > 0 ){
					foreach ($at_all_page_items as $key=>$service ){
						$page_ids[$key]['page_id'] = feminine_style_sanitize_page( $service['page_id'] );
					}
				}
			}
                ...
                <div class="container">
                    ...
                    <div <?php echo esc_attr( $div_attr ); ?>>
						...
						if( !empty( $post_in ) && is_array( $post_in)) :
							...
							$service_query = new WP_Query( $feminine_style_post_in_page_args );
							...
							if ( $service_query->have_posts() ):
								...
								while ( $service_query->have_posts() ):$service_query->the_post();
									...
									?>
                                    <div class="<?php echo esc_attr( $feminine_style_list_classes ); ?> column">
                                        <div class="single-item <?php echo esc_attr( $animation ); ?>">
                                            <div class="icon clearfix">
												<?php
												$feminine_style_icon = get_post_meta( get_the_ID(), 'feminine-style-featured-icon', true );
												$feminine_style_icon = isset( $feminine_style_icon ) ? esc_attr( $feminine_style_icon ) : '';

												echo '<a href="'.esc_url(get_permalink()).'" class="all-link">';
												if ( ! empty ( $feminine_style_icon ) ) { ?>
                                                    <i class="<?php echo esc_attr( $feminine_style_icon ); ?>"></i>
													<?php
												}
												else {
	...
	} // Class Feminine_Style_Service ends here
}
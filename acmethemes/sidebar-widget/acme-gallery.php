 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Feminine_Style_Gallery' ) ) {

    class Feminine_Style_Gallery extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                    ...
                    esc_html__('AT Gallery Section', 'feminine-style'),
                    /*Widget description*/
                    array(
                            'description' => esc_html__( 'Show Gallery Section.', 'feminine-style' )
                    )
            );
        }
        ...
        public function form( $instance ) {
            ...
            <br/>
            <small><?php esc_html_e( 'Add Page, Reorder and Remove. Please do not forget to add Featured Image on selected pages. ', 'feminine-style' ); ?></small>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'image_popup_type' ) ); ?>"><?php esc_html_e( 'Image Popup Type', 'feminine-style' ); ?></label>
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'image_popup_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image_popup_type' ) ); ?>" >
                    <?php
                    $feminine_style_gallery_image_popup = feminine_style_gallery_image_popup();
                    foreach ( $feminine_style_gallery_image_popup as $key => $value ){
                        ?>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'feminine_style_img_size' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'feminine_style_img_size' ) ); ?>">
			        ...
			        foreach( $feminine_style_image_sizes as $key => $feminine_style_column_array ){
				        echo ' <option value="'.esc_attr( $key ).'" '.selected( esc_attr( $feminine_style_img_size ), $key, 0). '>'.esc_attr( $feminine_style_column_array ).'</option>';
			        }
			        ?>
                </select>
            </p>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        if( isset($new_instance['at_all_page_items'] )){
		        ...
		        if  ( is_array($at_all_page_items) && count($at_all_page_items) > 0 ){
			        foreach ($at_all_page_items as $key=>$about ){
				        $page_ids[$key]['page_id'] = feminine_style_sanitize_page( $about['page_id'] );
			        }
		        }
	        }
	        ...
	        $feminine_style_image_sizes              = feminine_style_get_image_sizes_options();
	        $instance[ 'feminine_style_img_size' ]   = feminine_style_sanitize_choice_options( $new_instance[ 'feminine_style_img_size' ], $feminine_style_image_sizes, 'large' );

	        $feminine_style_gallery_image_popup      = feminine_style_gallery_image_popup();
	        $instance[ 'image_popup_type' ]         = feminine_style_sanitize_choice_options( $new_instance[ 'image_popup_type' ], $feminine_style_gallery_image_popup, 'gallery' );
            ...
            <section id="<?php echo esc_attr( $unique_id ); ?>" class="at-widgets acme-gallery">
                <div class="full-width-container">
                    ...
                    if( !empty ( $post_in ) && is_array( $post_in )) :
	                    ...
	                    $gallery_query = new WP_Query( $feminine_style_post_in_page_args );
                        ?>
                        <div class="row fullwidth-row">
                            ...
                            if ( $gallery_query->have_posts() ):
                                ...
                                while( $gallery_query->have_posts() ):$gallery_query->the_post();
	                                if( 1 == $column_number ){
		                                $feminine_style_column = " col-xs-12";
	                                }
                                    elseif( 2 == $column_number ){
		                                $feminine_style_column = " col-sm-6";
	                                }
                                    elseif( 3 == $column_number ){
		                                $feminine_style_column = " col-sm-4 col-md-4";
	                                }
                                    elseif( 4 == $column_number ){
		                                $feminine_style_column = ' col-sm-3 col-md-3';
	                                }
	                                else{
		                                $feminine_style_column = " col-sm-3 col-md-3";
	                                }
                                    ?>
                                    <div class="at-gallery-item <?php echo esc_attr( $feminine_style_column ); ?>">
                                        <div class="gallery-inner-item <?php echo esc_attr( $animation );?>">
                                            <div class="at-bottom-lower">
                                                ...
                                                if( 'disable' != $image_popup_type ){
                                                    ...
                                                    <a class="<?php echo esc_attr( $image_popup_class );?>" href="<?php echo esc_url( $image_url_full[0] );?>">
                                                        <i class="fas fa-search gallery-button primary-bg"></i>
                                                    </a>
	                                                <?php
                                                }
                                                ?>
                                            ...
                                            if( has_post_thumbnail() ):
                                                $image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), $feminine_style_img_size );
                                            else:
    ...
    } // Class Feminine_Style_Gallery ends here
}